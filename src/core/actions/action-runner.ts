      ...
      try {
        ...
        if (currentResult.crucialOutputPaths && currentResult.crucialOutputPaths.length > 0) {
          ...
          for (const outputPath of currentResult.crucialOutputPaths) {
            log.message(`  - ${path.resolve(outputPath)}`);
          }
          log.message("");
        }
        ...
        spin.stop(`Failed action: ${action.name}`);
        log.error(`${error}`);
...
// Helper function to execute a single action and log its timing and output
export async function executeAndLogAction(
  action: Action, 
  ...
  if (actionResult.outputPaths && actionResult.outputPaths.length > 0) {
    ...
    for (const outputPath of actionResult.outputPaths) {
